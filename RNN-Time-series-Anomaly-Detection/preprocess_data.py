...
class PickleDataLoad(object):
    def __init__(self, data_type, filename, augment_test_data=True):
        ...
        self.trainData, self.trainLabel = self.preprocessing(Path('train_dataset'),train=True)
        self.testData, self.testLabel = self.preprocessing(Path('test_dataset'),train=False)
