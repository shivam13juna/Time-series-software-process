...
class RNNPredictor(nn.Module):
    ...
    def save_checkpoint(self,state, is_best):
        print("=> saving checkpoint ..")
        # args = state['args']
        checkpoint_dir = Path('save')
        checkpoint_dir.mkdir(parents=True,exist_ok=True)
        checkpoint = checkpoint_dir

        torch.save(state, 'save/bonhwa.pth')
        if is_best:
            model_best_dir = Path('save','model_best')
            model_best_dir.mkdir(parents=True,exist_ok=True)

            shutil.copyfile('save/bonhwa.pth', 'save/model_best/best_bonhwa.pth')
