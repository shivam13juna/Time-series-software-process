...
print("The shape of gen_dataset: ", gen_dataset.shape)
    ...
    if args.save_fig:
        ...
        with torch.no_grad():
            for i in range(endPoint):
                ...
                # model.train()
                # for i in range(20):
                #     out_, hidden_ = model.forward(out+0.01*Variable(torch.randn(out_.size())).cuda(),hidden,noise=True)
                #     outs.append(out_)
                # model.eval()
                ...
                #     print("inner getting on")
                #     out, hidden = model.forward(out, hidden)

                #     #print(out_mean,out)

                # else:
                #     print("outer getting on")
        ...
        print("So this is the shape of target", target.size(-1))
        for i in range(target.size(-1)):
            plt.plot(target[:,:,i].numpy(), label='Target'+str(i),
                     color='black', marker='.', linestyle='--', markersize=1, linewidth=0.5)
            # plt.plot(outSeq[:startPoint,i].numpy(), label='1-step predictions for target'+str(i),
            ...
            # plt.plot(range(startPoint, endPoint), lowerlim95[:,i].numpy(), label='lowerlim'+str(i),
                        # color='skyblue', marker='.', linestyle='--', markersize=1.5, linewidth=1)
            # print(outSeq.shape)
            plt.plot(outSeq[startPoint:,i].numpy()+target[:,:,i].numpy(),
        ...
        plt.savefig('result/'+'fig_epoch'+str(epoch)+'.png')
        #plt.show()
        plt.close()
        return outSeq
...
def evaluate(args, model, test_dataset):
    ...
    with torch.no_grad():
        ...
        for nbatch, i in enumerate(range(0, test_dataset.size(0) - 1, args.bptt)):
            ...
            # print(outSeq1)
            hids1 = torch.cat(hids1,dim=0)
            print("Shape of outseq1: ", outSeq1.shape,targetSeq.shape)
            loss1 = criterion(outSeq1.view(args.batch_size,-1), targetSeq.view(args.batch_size,-1))
...
if not args.pretrained:
    ...
    try:
        for epoch in range(start_epoch, args.epochs+1):
            ...
            generate_output(args,epoch,model,gen_dataset,startPoint=1)
